/*
  # Fix Dealer Performance Metrics

  1. Changes
    - Update performance calculation functions to track bags sold by dealers to their customers
    - Modify get_performance_metrics to calculate based on customer transactions through dealers
    - Update monthly performance calculation to reflect actual dealer sales to customers
    - Add proper dealer-customer relationship tracking

  2. Performance Metrics Logic
    - Dealers earn points when their customers (builders/contractors) purchase bags
    - Performance metrics should show bags sold TO customers, not bags requested BY dealers
    - Monthly performance tracks dealer's sales to their customer base
*/

-- Update the calculate_monthly_performance function to properly track dealer sales to customers
...
BEGIN
  ...
  -- Calculate total points approved for customers through this dealer
  -- This represents bags sold BY the dealer TO customers
  SELECT COALESCE(SUM(amount), 0)
  INTO v_total_points
  FROM transactions
  WHERE dealer_id = p_dealer_id
    AND type = 'earned'  -- Points earned by customers
    AND status IN ('dealer_approved', 'approved')
    AND created_at >= v_start_date
    AND created_at < v_end_date;
  
  -- Calculate total bags (points / 10) - these are bags sold to customers
  v_total_bags := FLOOR(v_total_points / 10);
  
  -- Calculate total transactions (customer purchases through this dealer)
  SELECT COALESCE(COUNT(*), 0)
  INTO v_total_transactions
  FROM transactions
  WHERE dealer_id = p_dealer_id
    AND type = 'earned'  -- Only customer purchase transactions
    AND created_at >= v_start_date
    AND created_at < v_end_date;
  
  -- Calculate unique customers who purchased through this dealer
...
-- Update the get_performance_metrics function to properly calculate dealer sales
...
BEGIN
  ...
  CASE p_period
    ...
    WHEN 'lifetime' THEN
      -- Get all-time performance from customer transactions through this dealer
      RETURN QUERY
      SELECT 
        'Lifetime'::text,
        COALESCE(FLOOR(SUM(CASE WHEN t.status IN ('dealer_approved', 'approved') AND t.type = 'earned' THEN t.amount ELSE 0 END) / 10), 0)::bigint,
        COALESCE(SUM(CASE WHEN t.status IN ('dealer_approved', 'approved') AND t.type = 'earned' THEN t.amount ELSE 0 END), 0)::bigint,
        COALESCE(COUNT(CASE WHEN t.type = 'earned' THEN 1 END), 0)::bigint,
        COALESCE(COUNT(DISTINCT CASE WHEN t.type = 'earned' THEN t.user_id END), 0)::bigint,
        (SELECT MIN(created_at)::date FROM transactions WHERE dealer_id = p_dealer_id AND type = 'earned'),
        v_current_date
      FROM transactions t
      WHERE t.dealer_id = p_dealer_id;
    
    WHEN 'custom' THEN
      -- Use provided start and end dates for customer transactions
...
-- Update the trigger function to only track customer transactions (not dealer self-requests)
CREATE OR REPLACE FUNCTION trigger_update_monthly_performance() RETURNS trigger AS $$
DECLARE
  v_year integer;
  v_month integer;
BEGIN
  -- Only update performance for customer transactions (earned type with dealer_id)
  -- Skip dealer self-requests (where user_id = dealer_id)
  
  IF TG_OP = 'DELETE' THEN
    v_year := EXTRACT(YEAR FROM OLD.created_at);
    v_month := EXTRACT(MONTH FROM OLD.created_at);
    
    -- Only update if this was a customer transaction through a dealer
    IF OLD.dealer_id IS NOT NULL AND OLD.type = 'earned' AND OLD.user_id != OLD.dealer_id THEN
  ...
  ELSE
    ...
    -- Only update if this is a customer transaction through a dealer
    IF NEW.dealer_id IS NOT NULL AND NEW.type = 'earned' AND NEW.user_id != NEW.dealer_id THEN

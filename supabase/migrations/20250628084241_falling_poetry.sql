/*
  ...
  1. Changes
     - Safely drop and recreate policies on the notifications table
     - Add policy to allow system to create notifications for any user
     - Update transaction notification function with SECURITY DEFINER
  
  2. Security
     - Ensures proper RLS policies for notifications
     - Maintains user data privacy while allowing system notifications
*/
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'notifications' 
    AND policyname = 'Service role has full access to notifications'
  ) THEN
    DROP POLICY "Service role has full access to notifications" ON notifications;
  END IF;
  
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'notifications' 
    AND policyname = 'System can create notifications for any user'
  ) THEN
    DROP POLICY "System can create notifications for any user" ON notifications;
  END IF;
END
$$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE tgname = 'transaction_notification_trigger' 
    AND tgrelid = 'transactions'::regclass
  ) THEN
    DROP TRIGGER transaction_notification_trigger ON transactions;
  END IF;
END
$$;
